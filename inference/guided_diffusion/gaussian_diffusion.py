...
try:
    from improved_diffusion.scheduler import get_schedule_jump
except ImportError:
...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        # Handle special values for schedule_jump_params
        schedule_jump_params = conf.schedule_jump_params
        if isinstance(schedule_jump_params, str):
            if schedule_jump_params.lower() in ['none', 'null', '']:
        ...
        if schedule_jump_params:
            ...
            # Standard DDPM sampling (no jumping)
    ...
    def _predict_eps_from_xstart(self, x_t, t, pred_xstart):
        """
        Predict epsilon from predicted x_start.
        """
        return (
    ...
    def ddim_sample(
        ...
        """
        # Image inpainting mask injection logic (consistent with p_sample)
        if conf is not None and conf.get('inpa_inj_sched_prev', False):
            if pred_xstart is not None and model_kwargs is not None:
                gt_keep_mask = model_kwargs.get('gt_keep_mask')
                if gt_keep_mask is None:
                    gt_keep_mask = conf.get_inpa_mask(x)
                ...
                if conf.get('inpa_inj_sched_prev_cumnoise', False):
                    weighed_gt = self.get_gt_noised(gt, int(t[0].item()))
                else:
                    gt_weight = th.sqrt(alpha_cumprod)
                    gt_part = gt_weight * gt
        ...
        return {"sample": sample, "pred_xstart": out["pred_xstart"], 'gt': model_kwargs.get('gt') if model_kwargs else None}
    ...
    def ddim_sample_loop_progressive(
        ...
        # In respace mode, self.num_timesteps is already the resampled step count (e.g., 250)
        # We use indices from 0 to num_timesteps-1
        indices = list(range(self.num_timesteps))[::-1]
