...
class Default_Conf(NoneDict):
    ...
    def get_dataloader(self, dset='train', dsName=None, batch_size=None, return_dataset=False, skip_existing=False):
        ...
        if ds_conf.get('mask_loader', False):
            if skip_existing:
                from guided_diffusion.image_datasets import load_data_inpa_skip_existing
                return load_data_inpa_skip_existing(**ds_conf, conf=self, skip_existing=True)
            else:
