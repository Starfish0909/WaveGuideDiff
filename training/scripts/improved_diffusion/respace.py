...
def resample_betas(original_betas, new_num_steps):
    """
    Resample beta schedule to a different number of steps.

    :param original_betas: numpy array of original beta values
    :param new_num_steps: desired number of steps
    :return: resampled beta values
    """
    original_betas = np.array(original_betas)
    original_num_steps = len(original_betas)

    # Create indices for resampling
    indices = np.linspace(0, original_num_steps - 1, new_num_steps)

    # Interpolate beta values
    new_betas = np.interp(indices, np.arange(original_num_steps), original_betas)
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, conf=None, **kwargs):
        ...
        # Add safety check
        if conf is not None and hasattr(conf, 'respace_interpolate') and conf.respace_interpolate:
        ...
        # Add safety check
        if conf is not None and hasattr(conf, 'use_value_logger') and conf.use_value_logger:
...
class _WrappedModel:
    ...
    def __call__(self, x, ts, **kwargs):
        ...
        # Add safety check
        if self.conf is not None and hasattr(self.conf, 'respace_interpolate') and self.conf.respace_interpolate:
            new_ts = new_ts.float() * (
