...
from .nn import (
    ...
    normalization,
    timestep_embedding,
    checkpoint as nn_checkpoint,  # Renamed to avoid conflict
)
...
import torch
import torch.nn as nn
import torch.utils.checkpoint as checkpoint  # PyTorch checkpointing module
...
class BasicLayer(TimestepBlock):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, time_embed_dim=128 * 4,
                 out_channels=64):
...
class BasicLayer_up(TimestepBlock):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.input_resolution = input_resolution
        self.depth = depth
        self.use_checkpoint = False  # Disabled to avoid DDP conflicts
        ...
        if upsample is not None:
            self.upsample = TimestepEmbedSequential(
                PatchExpand(input_resolution, dim=dim, dim_scale=2, norm_layer=norm_layer))
        else:
            self.upsample = None
...
class SwinUNetModel(nn.Module):
        ...
        for i_layer, mult in enumerate(self.depths):
            for _ in range(num_res_blocks):
                layers = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                                    ...
                                    out_channels=max(1, int(64 / (pow(2, i_layer)))),  # Ensure non-zero
            ...
            for _ in range(num_res_blocks):
                ...
                if i_layer == 0:
                    ...
                    layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                             ...
                                             use_checkpoint=use_checkpoint,
                                             out_channels=max(1, int(64 / (pow(2, 3 - i_layer)))),  # Ensure non-zero
    ...
    def forward(self, x, timesteps, y=None, **kwargs):
        # kwargs used to accept extra parameters (like clean_image), but base model doesn't use them
    ...
    def get_feature_vectors(self, x, timesteps, y=None):
        ...
        """
        # Consistent with forward: first through patch_embed and pos_drop
        x = self.patch_embed(x)
        x = self.pos_drop(x)
        ...
        for inx, module in enumerate(self.input_blocks):
            hs.append(h)  # Save first, then process (consistent with forward)
            h = module(h, emb)
...
class SwinUNetSuperResModel(SwinUNetModel):
